...
# Usage: python batch_formation_energy.py <algorithm_name> <cif_parent_folder> <output_root>
if len(sys.argv) < 4:
    print("Usage: python batch_formation_energy.py <algorithm> <cif_parent_folder> <output_root>")
    sys.exit(1)

ALGORITHM = sys.argv[1]
CIF_FOLDER = sys.argv[2]
...
def get_reference_energies(formula, optimizer):
    ...
    refs = {}
    temp_atoms = formula if isinstance(formula, Atoms) else Atoms(formula)
...
# --- 3. Process Folders ---
# Get all subdirectories in the parent CIF folder
formula_folders = [f.name for f in os.scandir(CIF_FOLDER) if f.is_dir()]

for formula in formula_folders:
    folder_path = os.path.join(CIF_FOLDER, formula)
    
    # Generate target references based on the folder name
    try:
        temp_target = Atoms(formula)
    except Exception as e:
        print(f"Skipping directory '{formula}' - Cannot parse as a valid chemical formula: {e}")
        continue
    ...
    if not cif_files:
        print(f"No valid unrelaxed CIF files found in {folder_path}, skipping...")
        continue
    ...
    num_to_save = max(1, int(len(results) * float(TOP_PICK)))
    top_results = results[:num_to_save]

    # Create output directory: {formula}
